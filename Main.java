    ...
    public void start(Stage rootStage) {
        ...
        volumeSlider.valueChangingProperty().addListener(new ChangeListener<Boolean>() {
            ...
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                currentSong[0].setVolume(volumeSlider.getValue());
            }
        });
        ...
        nextSongInPlaylistButton.setOnAction(event -> {
            currentSong[0].seek(currentSong[0].getTotalDuration().add(new Duration(2000)));
        });
        ...
        loadPlaylistButton.setOnAction(event -> {
            ...
            AnimationTimer advancePlaylist = new AnimationTimer() {
                @Override
                public void handle(long now) {
                    currentSong[0].setOnEndOfMedia(new Runnable() {
                        @Override
                        public void run() {
